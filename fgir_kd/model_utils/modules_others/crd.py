...
https://github.com/AberHu/Knowledge-Distillation-Zoo/blob/master/kd_losses/crd.py
'''
class CRD(nn.Module):
    '''
    ...
    '''
    def __init__(self, n_data, s_dim, t_dim, feat_dim=128, pool='gap', shape_format=False,
                nce_n=4096, nce_t=0.1, nce_mom=0.5):
        super(CRD, self).__init__()
        self.embed_s = Embed(s_dim, feat_dim)
        self.embed_t = Embed(t_dim, feat_dim)

        if pool == 'gap' and shape_format == 'bsd':
            self.pool = Reduce('b s d -> b d', 'mean')
        elif pool == 'gap' and shape_format == 'bdhw':
            self.pool = Reduce('b d h w -> b d', 'mean')
        elif pool == 'gap' and shape_format == 'bhwd':
            self.pool = Reduce('b h w d -> b d', 'mean')
...
class CRDLoss(nn.Module):
    ...
    Args:
        ...
        n_data: the number of samples in the training set, which is the M in Eq.(19)
    '''
    def __init__(self, n_data=10000, feat_dim=128, nce_n=4096, nce_t=0.1, nce_mom=0.5):
